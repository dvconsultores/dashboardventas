...
from .dashboard_serializer_distribuidoras import *
...
def index(request):
    return render(request, "dashboardventas/index.html")  
...
####################################################################################
#########################DISTRIBUIDORAS NEGOCIO#####################################
####################################################################################       

#@login_required(login_url=login_required_ruta)
def jsonVtasVolumendEntero(request):
    jsonData = json_parser_volumend()
    return JsonResponse(jsonData, safe=False)

#@login_required(login_url=login_required_ruta)
def jsonVtasPreciodEntero(request):
    jsonData = json_parser_montod()
    return JsonResponse(jsonData, safe=False) 

#@login_required(login_url=login_required_ruta)
def jsonVtasTotaldEntero(request):
    jsonData = json_parser_totald()
    return JsonResponse(jsonData, safe=False)    
